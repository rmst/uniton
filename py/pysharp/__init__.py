...
class Unity(Namespace):
  def __init__(self):
    c = Connection()
    super().__init__(c)
    self._default_timescales = (1., 0.02, 0.)  # the real values will be set on stepping=True

  @property
  def stepping(self):
    return self._con.u.stepping.py()
  ...
  def stepping(self, v):
    ...
    if v:
      ...
      t.timeScale = 100
      t.fixedDeltaTime = 0.01
      t.captureFramerate = 1000
    else:
      (t.timeScale, t.fixedDeltaTime, t.captureFramerate) = self._default_timescales
    self._con.u.setStepping(v)
